...
const PhoneShop = () => {
  ...
  const appliancesCategory = productData.find(
    (category) =>
      category.category_name.toLowerCase() === 'appliances'.toLowerCase()
  );

  if (!appliancesCategory || appliancesCategory.products.length === 0) {
    return <div>No Home Appliances available</div>; // Handle case where no televisions are found
  }
  ...
  return (
    <div>
      ...
      <div className="phone-shop">
        <h1>Our Home Appliances Collection</h1>

        <div className="category-container">
          {/* Display Televisions category */}
          <div key={appliancesCategory.category_name} className="category">
            <h2>{appliancesCategory.category_name}</h2>
            <p>{appliancesCategory.category_description}</p>
            <div className="phone-list">
              {/* Iterate over products in the 'Televisions' category */}
              {appliancesCategory.products.map((product) => (
