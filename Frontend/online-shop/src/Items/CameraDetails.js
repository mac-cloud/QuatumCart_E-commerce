...
const PhoneShop = () => {
  ...
  const camerasCategory = productData.find(
    (category) =>
      category.category_name.toLowerCase() === 'cameras'.toLowerCase()
  );

  if (!camerasCategory || camerasCategory.products.length === 0) {
  ...
  return (
    <div>
      ...
      <div className="phone-shop">
        <h1>Our Cameras Collection</h1>

        <div className="category-container">
          {/* Display Televisions category */}
          <div key={camerasCategory.category_name} className="category">
            <h2>{camerasCategory.category_name}</h2>
            <p>{camerasCategory.category_description}</p>
            <div className="phone-list">
              {/* Iterate over products in the 'Televisions' category */}
              {camerasCategory.products.map((product) => (
