...
const PhoneShop = () => {
  ...
  const cablesCategory = productData.find(
    (category) =>
      category.category_name.toLowerCase() === 'cables'.toLowerCase()
  );

  if (!cablesCategory || cablesCategory.products.length === 0) {
    return <div>No Cables available</div>; 
  }
  ...
  return (
    <div>
      ...
      <div className="phone-shop">
        <h1>Our Cables Collection</h1>

        <div className="category-container">
          {/* Display Televisions category */}
          <div key={cablesCategory.category_name} className="category">
            <h2>{cablesCategory.category_name}</h2>
            <p>{cablesCategory.category_description}</p>
            <div className="phone-list">
              {/* Iterate over products in the 'Televisions' category */}
              {cablesCategory.products.map((product) => (
